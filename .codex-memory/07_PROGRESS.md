...
1. Закоммитить и запушить накопленные правки.
2. Протестировать форму на опубликованном домене после GitHub Pages обновления.
3. Проверить сценарий обычной отправки: PIN сотрудника → текущая дата → отправка → запись в строку даты.
4. Проверить сценарий ретроввода: старая дата → PIN руководителя → выбор сотрудника → отправка → перезапись строки даты.
5. Убедиться, что отчёт попадает в Google Sheets и Telegram.
6. Перенести реальные PIN из публичного кода в управляемые настройки без публичных дефолтов.
7. Добавить автопродление календарного каркаса после 2026-08-31 или отдельную безопасную maintenance-команду.

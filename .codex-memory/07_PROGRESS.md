...
- Текущий публичный Apps Script deployment обновлён до версии `35` (`Codex simplify sheet money format`).
...
- В live `Данные` удалены строки после 31.08.2026; grid теперь 126 строк.
- Скрыты detail/audit блоки: `ОТКРЫТИЕ`, `РАСХОДЫ`, `СВЕРКА КАССЫ`.
- Денежные значения в таблице приведены к целым числам без `₽`, копеек и разделителей.
- A:B подсвечивает пятницу, субботу и воскресенье через conditional formatting.
- Границы месяцев закреплены жирной горизонтальной линией.
...
1. Пользователь визуально проверяет live `Данные` после новых табличных правок.
2. Проверить, что новая отправка формы не возвращает `₽`/копейки в перезаписанную строку.
3. Убедиться, что отчёт попадает в Google Sheets и Telegram.
4. Перенести реальные PIN из публичного кода в управляемые настройки без публичных дефолтов.
5. Добавить автопродление календарного каркаса после 2026-08-31 или отдельную безопасную maintenance-команду.
6. Добавить PWA manifest для установки формы как приложения.
7. Добавить offline queue / повтор отправки при плохой связи.
8. Добавить историю смен в форме.
